...
| *(no tester)* | verifying that it RUNS is a `kind="code"` phase over `quality.ts`, not an agent | `QualityResult` → `as_envelope` | the exit code is the check |
...
export async function main(
  ...
  const cfg = agents.load_config(config);         // 1. point to config
  agents.validate(cfg, REQUIRED_AGENTS);          // 2. fail fast — nothing spawns on a half-valid config
  const run = session.ensure(cfg, adw_id);        // 3. pin-or-create the session → the Run object
...
**The phase is a callback, not a block.** `run.phase(params, body)` awaits the body, and the phase succeeds only when the body returns — a throw anywhere inside it records the failure and re-raises, so success is still earned rather than assumed. Whatever the body returns is what `await run.phase(...)` gives you, which is how an envelope reaches the next phase.
