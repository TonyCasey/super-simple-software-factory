...
A code phase does its work in the callback body and logs what it did. Whatever the body returns is what `await run.phase(...)` yields, which is how an envelope reaches the next phase. The commit phase that closes `adw_plan_build.ts` and `adw_plan_build_test.ts` is the pattern:
...
`commit_message` is a field on `PlanOutput`, `BuildOutput`, and `DocumentOutput` that the agent fills in **for its own work product**, so always pair it with a fallback — it defaults to empty. `commit_all` throws if the cwd is not a git repo or nothing changed, which fails the phase rather than committing nothing. A chain that commits more than once (`adw_simple_sdlc.ts`) commits each product with its own author's message.
...
```typescript
    (ph) => ph.call(AgentCall({
      output_type: BuildOutput, prompt, previous: plan,
...
const MAX_FIX_LOOPS = 3;
  ...
  for (let i = 1; i <= MAX_FIX_LOOPS; i++) {
    test = await run.phase(
      PhaseParams({ name: `test_${i}`, kind: "code", owner: "quality",
                    description: "Run the suite — a known command, so code runs it" }),
      async (ph) => {
        const result = await quality.run_tests(run);   // QualityResult, not an envelope
    ...
    await run.phase(
      PhaseParams({ name: `fix_${i}`, kind: "agent", owner: "builder", retries: 1,
                    description: "Repair what the suite reported, from its verbatim output" }),
      (ph) => ph.call(AgentCall({
        output_type: BuildOutput, prompt,
...
its tests, in the db and the UI as well as the terminal. Pass the acceptance flag and
