...
1. **Auth** — the starter roster is entirely `coding_agent: claude_code`, so there is **no key to set**: run `claude login` once (or `claude setup-token` on a headless box) and every agent bills your Claude Pro/Max subscription. Do **not** put `ANTHROPIC_API_KEY` in `.env` if you want subscription billing — Claude Code prefers an API key over the subscription, which is exactly why `agent_cc.ts` strips it from the child environment. You only need `cp .env.sample .env` and a provider key once you add a `coding_agent: pi` agent.
