...
| `agent_codex.ts` | the Codex interface — `codex exec --json` on a ChatGPT subscription; `writes: []` becomes a read-only sandbox, and models are allowlisted because a ChatGPT plan serves fewer than the API does |
